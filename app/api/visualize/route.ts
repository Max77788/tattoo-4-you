...
export async function POST(request: Request) {
  try {
    ...
    if (!encoded) return NextResponse.json({ error: "Nara returned no preview image." }, { status: 502 });
    return NextResponse.json({ image: `data:image/png;base64,${encoded}`, model: process.env.NARA_IMAGE_MODEL || "gpt-image-2" });
  } catch (error) {
